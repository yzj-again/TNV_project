...
int cache_c::get(char const *key, acl::string &value) const
{
    ...
    logger_warn("get cache ok,key: %s, value:%s",
                tracker_key.c_str(), value.c_str());
    g_rconns->put(rconn, true); // 连接放回连接池
    return OK;
}
// 设置指定键的值 查找键超时
int cache_c::set(char const *key, char const *value, int timeout) const
{
    ...
    acl::redis redis;
    redis.set_client(rconn);
    // 借助Redis客户机根据键设置指定键的值 setex(键,值，键超时)
    if (timeout < 0)
