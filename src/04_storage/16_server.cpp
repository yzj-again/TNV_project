...
void server_c::proc_on_init(void)
{
	...
	if (strlen(cfg_gpname) > STORAGE_GROUPNAME_MAX)
		logger_fatal("groupname too big %zu > %d", strlen(cfg_gpname), STORAGE_GROUPNAME_MAX);
	...
	// 创建并启动连接每台跟踪服务器的客户机线程 客户机和跟踪服务器通信
			 ...
			 taddr != g_taddrs.end(); ++taddr)
	{
		...
		tracker->set_detachable(false); // 可汇合的，方便回收
		tracker->start();
		m_trackers.push_back(tracker); // 收集指针，每一台跟踪服务器都启动了一个线程
	}
...
bool server_c::proc_exit_timer(size_t nclients, size_t nthreads)
{
	...
	{
		logger("nclients: %zu, nthreads: %zu", nclients, nthreads);
		return true;
	}
