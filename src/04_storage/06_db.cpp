...
int db_c::connect()
{
    ...
    for (std::vector<std::string>::const_iterator maddr =
        ...
        if ((m_mysql = mysql_real_connect(mysql, maddr->c_str(), "root", "12345678", "tnv_storagedb", 0, NULL, 0)))
            return OK;
...
int db_c::get(char const *appid, char const *userid, char const *fileid, std::string &filepath, long long *filesize) const
{
    ...
    // uid:fid:<用户ID>:<文件ID> <k-v> <文件大小>;<文件路径>
    cache_c cache;
    acl::string key;
